...
var url = (window.URL && URL.createObjectURL) ? URL : window.webkitURL;
...
export default class Exploder {
  ...
  explode(buffer) {
    ...
    return new Promise((resolve, reject) => {
      ...
      var width = streamReader.readLittleEndian(2),
        height = streamReader.readLittleEndian(2);
...
//        streamReader.log("SKIPPING " + blockSize + " BYTES");
      ...
      for (var i = 0; i < frames.length; i++) {
        ...
        let blob = new Blob([ gifHeader, buffer.slice(frame.index, nextIndex), gifFooter ], {type: 'image/gif'});
        frame.url = url.createObjectURL(blob);
      }

      console.debug("Received " + frames.length + " frames of gif " + this.file)
      resolve(new Gif(width, height, frames));
    })
  }
}
