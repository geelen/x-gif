...
import Renderer from './x-gif/renderer'
import Strategies from './x-gif/strategies'
import template from './x-gif.html!text'
...
class XGifController {
  ...
  srcChanged(src) {
    ...
    var exploder = new Exploder(src)
    exploder.load().then(gif => {
      this.renderer = new Renderer(gif, this.shadow.querySelector('#frames'), this.xgif.options.fill)
      this.playback = new Playback(gif, this.renderer, this.xgif.options);
  ...
  relayout() {
    if (this.renderer && this.xgif.options.fill) {
      this.renderer.scaleToFill();
    }
  }
}
